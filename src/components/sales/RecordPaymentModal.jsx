...
export default function RecordPaymentModal({ isOpen, onClose, saleDoc, onSuccess }) {
  const [amount, setAmount] = useState(saleDoc?.balanceDue || 0);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await client.post(`/sales/${saleDoc._id}/payments`, {
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-100 pb-4">
          <div>
            ...
            <p className="text-xs text-slate-500">Invoice: {saleDoc.documentNumber}</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          <div className="rounded-xl bg-slate-50 p-3 text-xs">
            <div className="flex justify-between text-slate-600">
              <span>Customer:</span>
              <span className="font-semibold text-slate-900">{saleDoc.customerName || saleDoc.customer?.name}</span>
            </div>
            <div className="mt-1 flex justify-between text-slate-600">
              <span>Total Invoice Amount:</span>
              <span className="font-semibold text-slate-900">₹{saleDoc.grandTotal?.toFixed(2)}</span>
            </div>
            <div className="mt-1 flex justify-between text-rose-600">
              <span>Outstanding Balance Due:</span>
              <span className="font-bold">₹{saleDoc.balanceDue?.toFixed(2)}</span>
            </div>
          </div>
          ...
          <div>
            ...
            >
              ...
              <option value="bankTransfer">Bank Transfer (NEFT/IMPS/UPI)</option>
